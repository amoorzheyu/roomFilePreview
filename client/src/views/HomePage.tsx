  ...
  return (
    <div className="min-h-[100dvh]">
      <div className="pointer-events-none fixed inset-0 opacity-80">
        <div className="absolute -left-40 -top-40 h-[520px] w-[520px] rounded-full bg-emerald-500/12 blur-3xl" />
        <div className="absolute -bottom-52 right-[-220px] h-[620px] w-[620px] rounded-full bg-sky-500/10 blur-3xl" />
      ...
      <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-10 px-6 py-14 md:grid-cols-[1.35fr_0.65fr] md:gap-12 md:px-10">
        <div className="relative">
          ...
          <h1 className="mt-6 text-4xl font-semibold tracking-tight text-zinc-50 md:text-6xl md:leading-[0.98]">
            多人同步预览
            <span className="block text-zinc-200/80">PDF / Markdown</span>
          </h1>

          <p className="mt-5 max-w-[62ch] text-base leading-relaxed text-zinc-200/70">
            创建房间后上传文件，房主滚动位置会实时同步给房间内的其他人。你可以随时停止共享、清空内容或覆盖上传新版本。
          </p>
          ...
          <div className="rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(255,255,255,0.08)]">
            <div className="grid gap-3">
              ...
              <button
                ...
                onClick={onCreate}
                disabled={busy !== null}
                className="group inline-flex w-full items-center justify-between rounded-2xl bg-emerald-500/90 px-4 py-3 text-sm font-semibold text-emerald-950 transition active:translate-y-[1px] disabled:opacity-60"
              >
            ...
            <div className="grid gap-3">
              <div className="text-sm font-medium text-zinc-100">进入房间</div>
              ...
              <div className="text-xs text-zinc-200/50">
                如果你是创建者，进入后会自动尝试恢复控制权（本机保存的 token）。
              </div>
            </div>
          </div>

          <div className="mt-4 text-xs text-zinc-200/45">
            该版本默认不做持久化：服务端重启后房间与内容会消失。
          </div>
        </div>
      </div>
    </div>
